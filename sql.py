...
Generate more data for `student` and `mark` tables, create a complex SQL query to filter and aggregate data, and then submit the resulting query via the Hillel LMS system.

#### Phase 0: Repeat, what we did during the lesson

Choose any SQL database (I did Postgres on the lesson), and execute all the queries:
...
1. **Insert More Students**: Add at least 5 more rows into the `student` table. Make sure to set the `is_active` column for some of them.

2. **Insert More Marks**: Add at least 10 more rows into the `mark` table with various `student_id` values.

#### Phase 2: SQL Query Creation

1. **Craft the Query**: Create a SQL query that joins the `student` and `mark` tables, filters by the `is_active` status, and aggregates the average marks, and then filters students, who have the average mark at least 70.

#### Phase 3: Submission

1. **Submission Time**: Once your SQL masterpiece is ready, and you've verified that it works as smooth as an bike on an open road, submit the resulting query using the Hillel LMS system for review.